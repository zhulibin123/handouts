globals [
  Global_Temperature    ; mean world temperature
  Temparature_Change    ; rate of change in temperature applied to each patch
  mean_Resilience       ; average tollerable temperature range
  mean_Influence        ; average influence on patch temperature for species
  year                  ; arbitary unit time
  ]

patches-own [
  temperature           ; local temperature at this location
]  

turtles-own [
  optimum_temperature  ; ideal temperature for species
  direction            ; whether agent warms or cools patch
  influence            ; amount by which agent warms or cools patch
  resilience           ; difference between ideal and current temperature that species can survive
]

to setup
  clear-all
  set temperature_change 0
  set year 0
  ask patches [
    set pcolor black
    set temperature random-normal 0 30
  ]
  new_species        ; put some agents on empty patches
end
...
to go
  ...
  set year year + 1
  ask patches [set temperature temperature + Temperature_Change]  ; external environmental change
  change_environment                                              ; environmental change caused by life
  survive                                                         ; remove species that cannot survive new temperature
  diffuse temperature diffusion_rate
  new_species
  set Global_Temperature mean [temperature] of patches
  set mean_Resilience mean [Resilience] of turtles
  set mean_Influence mean [Influence] of turtles
  update_plots
  ask patches [if temperature > 10 [set pcolor orange]]
  ask patches [if temperature > 20 [set pcolor red]]
  ask patches [if temperature < -10 [set pcolor turquoise]]
  ask patches [if temperature < -20 [set pcolor cyan]]
end
...
to new_species
  ...
  ask patches with [not any? turtles-here] [
    ...
    if r < 0.1 [
      sprout 1 [
        set optimum_temperature temperature
        set color white
        let d random-float 1
        if d <= 0.5 [set direction "warm" set heading 0]
        if d > 0.5 [set direction "cool" set heading 180]
        set influence random-exponential Initial_Influence 
        set resilience random-exponential Initial_Resilience 
      ]
    ]
  ]
end
  
to change_environment  ; species changes the temperature of the patch it is on by Influence
  ask turtles [
    if direction = "warm" [set temperature temperature + influence]
    if direction = "cool" [set temperature temperature - influence]
  ]
end
 
to survive
  ask turtles [
    if temperature < optimum_temperature + 5 and temperature > optimum_temperature - 5 [set color green]
    if temperature > optimum_temperature + 5 [set color magenta]
    if temperature < optimum_temperature - 5 [set color blue]
    if temperature > optimum_temperature + resilience [die]
    if temperature < optimum_temperature - resilience [die]
  ]
end
...
to update_plots
  set-current-plot "Global_Temperature"
  plot Global_Temperature
  
  set-current-plot "Temperature of Patches"
  histogram [temperature] of patches
end
@#$#@#$#@
GRAPHICS-WINDOW
402
...
false
PENS
"Global_Temperature" 1.0 0 -16777216 true
